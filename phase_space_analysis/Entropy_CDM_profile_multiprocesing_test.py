...
def mp_actions(stack_ids,part_pos,part_vel,ind,M200,R200,group_pos,count,part_mass,grav_smoothing,ids):
	nbins=30
	...
	for i in range(len(ids)):
		pos,vel,masss,conv_r=stacker(stack_ids[ids[i]],part_pos,part_vel,ind,M200,R200,group_pos,count,part_mass,grav_smoothing)	
...
location.append('/hpcdata4/sam/PhD/Investigating_Running/RUNS/DMONLY/L025N1024/run_m0p02473')
location.append('/hpcdata4/sam/PhD/Investigating_Running/RUNS/DMONLY/L025N1024/run_p0p00791')

ind,count,M200,R200,group_pos,part_pos,part_vel,part_mass=loader(location[0],snaps[0]) #load data
...
stack_ids,num,mass,rad,num_p=halo_to_stack(M200,R200,part_mass,num_stack=1500)	

cut=np.where(mass>mass_cut[0])[0] #cutting to only consider halos with a mass greater than 10**12 h^-1 M_sun
stack_idss=[]
...
n_p=np.array([1,2,4,8,16,32])
num_pp=np.logspace(np.log10(1500),np.log10(1000000),20)
times=np.empty((len(n_p),len(num_pp)))

for i in range(len(n_p)):
	print(i)
	for j in range(len(num_pp)):
		print(j)
		par=np.empty((len(mass),2))
		radius=np.empty((len(mass),nbins))
		profile=np.empty((len(mass),nbins))

		batch_n = 20000 #number of particles (summed N200) to be processed in each bath of the multiprocessing
		ids=num_batch(num_p,num_pp[j])

		mp=partial(mp_actions,stack_ids,part_pos,part_vel,ind,M200,R200,group_pos,count,part_mass,grav_smoothing)
		s_time=time.time()
		pool=Pool(processes=n_p[i])
		data=pool.map(mp,ids)

		# plt.figure()
		# plt.plot(data[0][1][0,:],data[0][2][0,:])
		# plt.show()
		# exit()
		for k in range(len(ids)):
			par[ids[k],:]=data[k][0]
			radius[ids[k],:]=data[k][1]
			profile[ids[k],:]=data[k][2]

		time_2=time.time()
		times[i,j]=time_2-s_time
...
np.save('Number_batch.npy',num_pp)
np.save('Number_process.npy',n_p)
