...
for j in range(num_sims):
	...
	for i in range(len(mass_range)-1):
		...
		bin_centret=np.logspace(np.log10(0.004),np.log10(4),200)
		rho_gausst,vel_gausst=phase_gaussian_kernel(pos_s[:,0],vel_s,1,bin_centret,m)

		m_less=mass_prof(pos_s[:,0],m,bin_centret)

		np.save('rho_gauss_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),rho_gausst)

		np.save('sig_gauss_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),vel_gausst)
		
		np.save('r_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),bin_centret)
		np.save('m_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),m_less)
		np.save('nhalo_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),len(stack_id))
		np.save('r_conv_%.1f_%.1f_%.1f_h.npy'%(cross_sections[j],label[i],label[i+1]),conv_rad)
		print(i)
