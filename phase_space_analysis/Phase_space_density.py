...
def phase_bin_constn(r,vel_s,R200,m,var_mass=False):
	...
	var=(var_r+var_th+var_ph)**0.5/3**0.5
...
def phase_gaussian_kernel(r,vel,R200,bin_centre,m,var_mass=False):
	...
	vel_dd=np.empty(len(bin_centre))
	...
	for i in range(len(bin_centre)):
		...
		vel_dd[i]=(vel_dispx+vel_dispy+vel_dispz)**0.5/3**0.5
...
def cartesian_to_spherical(pos,vel):
	...
	pos_spherical[:,1]=np.arccos(pos[:,0]/pos_spherical[:,0])
...
for i in range(len(mass_range)-1):
	...
	stack_id_0=np.where((M200_0<m_max) & (M200_0>m_min))[0]
	stack_id_1=np.where((M200_1<m_max) & (M200_1>m_min))[0]

	pos,vel,m=stacker(stack_id,part_pos,part_vel,ind,M200,R200,group_pos,count,part_mass)
	pos_0,vel_0,m_0=stacker(stack_id_0,part_pos_0,part_vel_0,ind_0,M200_0,R200_0,group_pos_0,count_0,part_mass)
	pos_1,vel_1,m_1=stacker(stack_id_1,part_pos_1,part_vel_1,ind_1,M200_1,R200_1,group_pos_1,count_1,part_mass)
	...
	rho_histt_0,vel_disp_histt_0,bin_centret_0=phase_bin_log(pos_s_0[:,0],vel_s_0,1,30,m_0,var_mass=True)
	rho_histt_1,vel_disp_histt_1,bin_centret_1=phase_bin_log(pos_s_1[:,0],vel_s_1,1,30,m_1,var_mass=True)

	bin_centret_h=np.logspace(np.log10(0.004),np.log10(1),500)
	...
	np.save('rho_gauss_0_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',rho_gausst)
	np.save('rho_gauss_0.1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',rho_gausst_0)
	np.save('rho_gauss_1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',rho_gausst_1)
	np.save('rho_squar_0_'+str(label[i])+'_'+str(label[i+1])+'.npy',rho_histt)
	np.save('rho_squar_0.1_'+str(label[i])+'_'+str(label[i+1])+'.npy',rho_histt_0)
	np.save('rho_squar_1_'+str(label[i])+'_'+str(label[i+1])+'.npy',rho_histt_1)

	np.save('sig_gauss_0_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',vel_gausst)
	np.save('sig_gauss_0.1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',vel_gausst_0)
	np.save('sig_gauss_1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',vel_gausst_1)
	np.save('sig_squar_0_'+str(label[i])+'_'+str(label[i+1])+'.npy',vel_disp_histt)
	np.save('sig_squar_0.1_'+str(label[i])+'_'+str(label[i+1])+'.npy',vel_disp_histt_0)
	np.save('sig_squar_1_'+str(label[i])+'_'+str(label[i+1])+'.npy',vel_disp_histt_1)
	
	np.save('r_0_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',bin_centret_h)
	np.save('r_0.1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',bin_centret_h)
	np.save('r_1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',bin_centret_h)

	np.save('m_0_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',m_less)
	np.save('m_0.1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',m_less_0)
	np.save('m_1_'+str(label[i])+'_'+str(label[i+1])+'_h.npy',m_less_1)

	np.save('nhalo_0_'+str(label[i])+'_'+str(label[i+1])+'.npy',len(stack_id))
	np.save('nhalo_0.1_'+str(label[i])+'_'+str(label[i+1])+'.npy',len(stack_id_0))
	np.save('nhalo_1_'+str(label[i])+'_'+str(label[i+1])+'.npy',len(stack_id_1))
	print(i)
