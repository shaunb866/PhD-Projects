...
def loader(loc):
	...
	h=h5.File(loc+'/groups_034/eagle_subfind_tab_034.0.hdf5')
	
	M_200=h['FOF']['Group_M_Crit200'][()]
	R_200=h['FOF']['Group_R_Crit200'][()]
	group_pos=h['FOF']['GroupCentreOfPotential'][()]
	for i in range(31):
		h=h5.File(loc+'/groups_034/eagle_subfind_tab_034.'+str(i+1)+'.hdf5')
...
def phase_gaussian_kernel(r,vel,R200,bin_centre,m):
	...
	vel=vel[np.where(r<2.0*R200)]
	m=m[np.where(r<2.0*R200)]
	r=r[np.where(r<2.0*R200)]
...
def stacker(stack_id,part_pos,part_vel,ind,M200,R200,group_pos,count,part_mass,grav_sm,box=100):
	...
	for i in range(len(stack_id)):
		...
		#calcuting convergence radius in r/R200
		a_circ=(np.arange(len(rr_less))+1)*part_mass/rr_less**2
		conv_rad[i]=rr_less[np.argmin(np.abs(a_circ-0.5*m2/(r2*grav_sm)))]/r2
...
def pow_const(x,a,b,c):
	return(a*(x)**(b)+c)
cross_sections=np.array([0,0.1,1,5,10])
num_sims=len(cross_sections)
location=[]
for i in range(num_sims):
	location.append('hpcdata4/arisbrow/simulations/L100N256_WMAP/DMONLY_SIDM%.1f' % cross_sections[i])


for i in range(num_sims):
	ind,count,M200,R200,group_pos,part_pos,part_vel,part_mass=loader(location[0]) #load data
...
sort=np.argsort(M200_0)
ind_0=ind_0[sort]; count_0=count_0[sort]; M200_0=M200_0[sort]; group_pos_0=group_pos_0[sort]; R200_0=R200_0[sort];
sort=np.argsort(M200_1)
ind_1=ind_1[sort]; count_1=count_1[sort]; M200_1=M200_1[sort]; group_pos_1=group_pos_1[sort]; R200_1=R200_1[sort];
...
# plt.figure()
# plt.plot(mass,num,'k.')
# plt.xlabel('M200 [h^-1 M_sun]',fontsize=12)
# plt.ylabel('Number in stack',fontsize=12)
# plt.xscale('log')
# #plt.yscale('log')
# plt.show()
# exit()

stack_ids_0,num_0,mass_0,rad_0=halo_to_stack(M200_0,R200_0,part_mass,num_stack=1500)
cut=np.where(mass_0>10**12)[0]	#cutting to only consider halos with a mass greater than 10**12 h^-1 M_sun
stack_idss=[]
for i in range(len(cut)):
	stack_idss.append(stack_ids_0[cut[i]])
stack_ids_0=stack_idss
num_0=num_0[cut]; mass_0=mass_0[cut]; rad_0=rad_0[cut];

stack_ids_1,num_1,mass_1,rad_1=halo_to_stack(M200_1,R200_1,part_mass,num_stack=1500)	
cut=np.where(mass_1>10**12)[0] #cutting to only consider halos with a mass greater than 10**12 h^-1 M_sun
stack_idss=[]
for i in range(len(cut)):
	stack_idss.append(stack_ids_1[cut[i]])
stack_ids_1=stack_idss
num_1=num_1[cut]; mass_1=mass_1[cut]; rad_1=rad_1[cut];

par=np.empty((len(mass),3))
par_0=np.empty((len(mass_0),3))
par_1=np.empty((len(mass_1),3))
radius=np.empty((len(mass),nbins))
radius_0=np.empty((len(mass),nbins))
radius_1=np.empty((len(mass),nbins))
profile=np.empty((len(mass),nbins))
profile_0=np.empty((len(mass),nbins))
profile_1=np.empty((len(mass),nbins))
# plt.figure()
for i in range(len(mass)):
	...
	bin_centre=np.logspace(np.log10(grav_smoothing/rad[i]),0,nbins)
	rho_gausst,vel_gausst,bin_centret=phase_gaussian_kernel(pos[:,0],vel,1,bin_centre,masss)
	
	params,err=curve_fit(pow_constl,bin_centre,np.log((vel_gausst[:,0]**3/rho_gausst*len(stack_ids[i]))**(2/3)),maxfev=10000000)
	par[i,:]=params
	profile[i,:]=(vel_gausst[:,0]**3/rho_gausst*len(stack_ids[i]))**(2/3)
	radius[i,:]=bin_centre
	# plt.plot(bin_centre,(vel_gausst[:,0]**3/rho_gausst/len(stack_ids[i]))**(2/3),'b')
	# plt.plot(bin_centre,pow_const(bin_centre,params[0],params[1],params[2]),'b--')
	# break
for i in range(len(mass_0)):
	print(i/len(mass_0))
	pos,vel,masss,conv_r=stacker(stack_ids_0[i],part_pos_0,part_vel_0,ind_0,M200_0,R200_0,group_pos_0,count_0,part_mass,grav_smoothing)
	pos,vel=cartesian_to_spherical(pos,vel)
	
	num_p=int(59.1*np.sum(pos[:,0]<1)**(0.32))
	bin_centre=np.logspace(np.log10(grav_smoothing/rad_0[i]),0,nbins)
	rho_gausst,vel_gausst,bin_centret=phase_gaussian_kernel(pos[:,0],vel,1,bin_centre,masss)
	
	params,err=curve_fit(pow_constl,bin_centre,np.log((vel_gausst[:,0]**3/rho_gausst*len(stack_ids_0[i]))**(2/3)),maxfev=10000000)
	par_0[i,:]=params
	profile_0[i,:]=(vel_gausst[:,0]**3/rho_gausst*len(stack_ids[i]))**(2/3)
	radius_0[i,:]=bin_centre
	# plt.plot(bin_centre,(vel_gausst[:,0]**3/rho_gausst/len(stack_ids_0[i]))**(2/3),'r')
	# plt.plot(bin_centre,pow_const(bin_centre,params[0],params[1],params[2]),'r--')
	# break
for i in range(len(mass_1)):
	print(i/len(mass_1))
	
	pos,vel,masss,conv_r=stacker(stack_ids_1[i],part_pos_1,part_vel_1,ind_1,M200_1,R200_1,group_pos_1,count_1,part_mass,grav_smoothing)
	pos,vel=cartesian_to_spherical(pos,vel)
	
	num_p=int(59.1*np.sum(pos[:,0]<1)**(0.32))
	bin_centre=np.logspace(np.log10(grav_smoothing/rad_1[i]),0,nbins)
	rho_gausst,vel_gausst,bin_centret=phase_gaussian_kernel(pos[:,0],vel,1,bin_centre,masss)
	
	params,err=curve_fit(pow_constl,bin_centre,np.log((vel_gausst[:,0]**3/rho_gausst*len(stack_ids_1[i]))**(2/3)),maxfev=10000000)
	par_1[i,:]=params
	profile_1[i,:]=(vel_gausst[:,0]**3/rho_gausst*len(stack_ids[i]))**(2/3)
	radius_1[i,:]=bin_centre
	# plt.plot(bin_centre,(vel_gausst[:,0]**3/rho_gausst/len(stack_ids_1[i]))**(2/3),'g')
	# plt.plot(bin_centre,pow_const(bin_centre,params[0],params[1],params[2]),'g--')
	# break
...
np.save('Entropy_mass_0.npy',mass)
np.save('Entropy_mass_0.1.npy',mass_0)
np.save('Entropy_mass_1.npy',mass_1)

np.save('Entropy_fit_params_0.npy',par)
np.save('Entropy_fit_params_0.1.npy',par_0)
np.save('Entropy_fit_params_1.npy',par_1)

np.save('Entropy_profile_0.npy',profile)
np.save('Entropy_profile_0.1.npy',profile_0)
np.save('Entropy_profile_1.npy',profile_1)

np.save('Rad_profile_0.npy',radius)
np.save('Rad_profile_0.1.npy',radius_0)
np.save('Rad_profile_1.npy',radius_1)

plt.figure()
plt.plot(mass,par[:,0],'k.',label='0')
plt.plot(mass_0,par_0[:,0],'g.',label='0.1')
plt.plot(mass_1,par_1[:,0],'r.',label='1')
plt.yscale('log')
plt.xscale('log')

plt.figure()
plt.plot(mass,par[:,1],'k.',label='0')
plt.plot(mass_0,par_0[:,1],'g.',label='0.1')
plt.plot(mass_1,par_1[:,1],'r.',label='1')
plt.yscale('log')
plt.xscale('log')

plt.figure()
plt.plot(mass,par[:,2],'k.',label='0')
plt.plot(mass_0,par_0[:,2],'g.',label='0.1')
plt.plot(mass_1,par_1[:,2],'r.',label='1')
plt.yscale('log')
plt.xscale('log')
